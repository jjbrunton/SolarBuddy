...
export function connectMqtt() {
  ...
  if (!settings.mqtt_host) {
    ...
    logMqttEvent('warning', 'Connection skipped because no MQTT host is configured.');
    return;
  }
  ...
  client.on('error', (err) => {
    ...
    logMqttEvent('error', `MQTT error: ${err.message}`);
  });

  client.on('close', () => {
    console.log('[MQTT] Connection closed');
    logMqttEvent('warning', 'Connection closed.');
    updateState({ mqtt_connected: false });
  });

  client.on('reconnect', () => {
    console.log('[MQTT] Reconnecting...');
    logMqttEvent('info', 'Reconnecting to broker.');
  });
}
...
export function disconnectMqtt() {
  if (g.__solarbuddy_mqtt) {
    g.__solarbuddy_mqtt.end(true);
    g.__solarbuddy_mqtt = null;
    logMqttEvent('info', 'Disconnected by application.');
    updateState({ mqtt_connected: false });
  }
}
