...
// Slots where the real, measured SolarBuddy behaviour cost more than a
// passive self-use battery would have. This is the best signal for "my plan
// is losing me money" — it isolates individual bad decisions rather than
// burying them inside a weekly average.
...
export function WorstSlotsPanel({ period }: { period: string }) {
  const [slots, setSlots] = useState<WorstSlot[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    fetch(`/api/analytics/worst-slots?period=${period}&limit=10`)
      .then((r) => r.json())
      .then((json) => {
        if (cancelled) return;
        setSlots(json.slots ?? []);
  ...
  const costlySlots = slots.filter((s) => s.delta > 0);

  return (
    <Card>
      <CardHeader
        title="Worst scheduling decisions"
        subtitle="Slots where the real plan cost more than a passive self-use battery would have. Priced against your actual half-hour tariff."
      />
      {loading && costlySlots.length === 0 ? (
        <p className="py-8 text-center text-sb-text-muted">Loading…</p>
      ) : costlySlots.length === 0 ? (
        <EmptyState
          title="No costly slots found"
          description="Over this period, SolarBuddy's decisions did not cost more than a passive self-use battery would have in any single slot."
        />
      ) : (
          ...
          {costlySlots.map((s) => (
            <li key={s.slot_start} className="grid grid-cols-[1fr_auto] items-start gap-4 py-3">
              ...
              <div className="text-right">
                <div className="font-mono text-[0.9rem] font-semibold text-sb-danger">
                  +{formatCost(s.delta)}
                </div>
